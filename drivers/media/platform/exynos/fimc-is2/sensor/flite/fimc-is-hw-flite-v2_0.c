...
#include "fimc-is-hw-flite-v2_0.h"
...
static void flite_hw_set_mux(u32 __iomem *base_reg, u32 csi_ch, u32 flite_ch)
{
	u32 cfg;
	if ((csi_ch == CSI_ID_A) && (flite_ch == FLITE_ID_B)) {
		info("[BNS] mux setting : CSI0 <-> BNS1\n");
		cfg = readl(base_reg + TO_WORD_OFFSET(FLITE_REG_CIGENERAL));
		cfg |= (1 << 12);
		writel(cfg, base_reg + TO_WORD_OFFSET(FLITE_REG_CIGENERAL));
	} else if ((csi_ch == CSI_ID_D) && (flite_ch == FLITE_ID_D)) {
		info("[BNS] mux setting : CSI3 <-> BNS3\n");
		cfg = readl(base_reg + TO_WORD_OFFSET(FLITE_REG_CIGENERAL));
		cfg |= (1 << 13);
		writel(cfg, base_reg + TO_WORD_OFFSET(FLITE_REG_CIGENERAL));
	}
...
static void flite_hw_set_mux_taa_isp(u32 __iomem *base_reg, u32 taa_ch, u32 isp_ch)
{
	u32 cfg;
...
u32 flite_hw_get_status(u32 __iomem *base_reg, u32 status_ids, bool clear)
{
	...
	for (i = 0; i < FLITE_STATUS_ALL; i++) {
		...
		case FLITE_STATUS_OFY:
			...
			break;
		case FLITE_STATUS_OFCR:
			if (status1 & (1 << 8)) {
				ret |= (1 << i);
				if (clear) {
					ciwdofst = readl(base_reg + TO_WORD_OFFSET(FLITE_REG_CIWDOFST));
					ciwdofst  |= (FLITE_REG_CIWDOFST_CLROVFICR);
					writel(ciwdofst, base_reg + TO_WORD_OFFSET(FLITE_REG_CIWDOFST));
					ciwdofst  &= ~(FLITE_REG_CIWDOFST_CLROVFICR);
					writel(ciwdofst, base_reg + TO_WORD_OFFSET(FLITE_REG_CIWDOFST));
				}
			...
			break;
		case FLITE_STATUS_OFCB:
			if (status1 & (1 << 9)) {
				ret |= (1 << i);
				if (clear) {
					ciwdofst = readl(base_reg + TO_WORD_OFFSET(FLITE_REG_CIWDOFST));
					ciwdofst  |= (FLITE_REG_CIWDOFST_CLROVFICB);
					writel(ciwdofst, base_reg + TO_WORD_OFFSET(FLITE_REG_CIWDOFST));
					ciwdofst  &= ~(FLITE_REG_CIWDOFST_CLROVFICB);
					writel(ciwdofst, base_reg + TO_WORD_OFFSET(FLITE_REG_CIWDOFST));
				}
		...
		int csi_ch, int flite_ch, int taa_ch, int isp_ch)
{
	...
	if (instance == 0) {
		...
		if (taa_ch == 0)
			otf_setting = FLITE_REG_CIGENERAL_3AA0_CAM_D;
		else if (taa_ch == 1)
			otf_setting = FLITE_REG_CIGENERAL_3AA1_CAM_D;
	} else {
	...
	if (taa_ch >= 0 && isp_ch >= 0)
		flite_hw_set_mux_taa_isp(base_reg, taa_ch, isp_ch);
}
...
void flite_hw_dump(u32 __iomem *base_reg)
{
	...
	info("[0x%04X] : 0x%08X\n", FLITE_REG_CIGENERAL, readl(base_reg + TO_WORD_OFFSET(FLITE_REG_CIGENERAL)));
