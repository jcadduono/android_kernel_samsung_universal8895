...
static void sensor_2l2_cis_data_calculation(const struct sensor_pll_info_compact *pll_info_compact, cis_shared_data *cis_data)
{
	...
	cis_data->min_frame_us_time = (u32)((u64)(pll_info_compact->frame_length_lines) * pll_info_compact->line_length_pck * 1000
					/ (vt_pix_clk_hz / 1000));
...
int sensor_2l2_cis_check_rev(struct fimc_is_cis *cis)
{
	...
	case 0xA2:
	case 0xA3:
		dbg_sensor("2l2 sensor Rev. 0x%X\n", rev);
		break;
	case 0xA0:
	case 0xB1:
	case 0xC0:
		dbg_sensor("Old version 2l2 sensor. Rev. 0x%X\n", rev);
		break;
	case 0x00:
...
int sensor_2l2_cis_retention_prepare(struct v4l2_subdev *subdev)
{
	...
	dbg_sensor("[%s] retention sensor sram write done\n", __func__);
...
int sensor_2l2_cis_stream_on(struct v4l2_subdev *subdev)
{
	...
	I2C_MUTEX_LOCK(cis->i2c_lock);
	sensor_2l2_cis_group_param_hold_func(subdev, 0x01);
	...
	sensor_2l2_cis_group_param_hold_func(subdev, 0x00);
	I2C_MUTEX_UNLOCK(cis->i2c_lock);
	...
	dbg_sensor("[%s] sens_config_index_cur=%d\n", __func__, mode);
	...
	switch (mode) {
		...
		case SENSOR_2L2_8064X1960_30FPS:
			...
			dbg_sensor("[%s] scene=%d, scenario_flag=%d\n", __func__, scene, dual_flag);

			if(dual_flag == true) {
				cis->cis_data->min_frame_us_time = (u32)((u64)(sensor_2l2_pllinfos[mode]->frame_length_lines + 0xA)
					* sensor_2l2_pllinfos[mode]->line_length_pck * 1000 / (sensor_2l2_pllinfos[mode]->pclk / 1000));
...
p_err:
	...
	case SENSOR_2L2_1008X756_120FPS:
		break;
	default:
		if (ext_info != NULL && ext_info->use_retention_mode != SENSOR_RETENTION_DISABLE) {
			ret = sensor_2l2_cis_stream_on(subdev);
			if (ret < 0)
				err("sensor_2l2_stream_on fail!!");
			ret = sensor_2l2_cis_stream_off(subdev);
			if (ret < 0)
				err("sensor_2l2_stream_off fail!!");
...
int sensor_2l2_cis_set_exposure_time(struct v4l2_subdev *subdev, struct ae_param *target_exposure)
{
	...
	dbg_sensor("[MOD:D:%d] %s, vsync_cnt(%d), vt_pic_clk_freq_mhz (%d), \
	...
	dbg_sensor("[MOD:D:%d] %s, vsync_cnt(%d), frame_length_lines(%#x), \
		long_coarse_int %#x, short_coarse_int %#x\n",
...
int sensor_2l2_cis_set_digital_gain(struct v4l2_subdev *subdev, struct ae_param *dgain)
{
	...
	dbg_sensor("[MOD:D:%d] %s(vsync cnt = %d), input_dgain = %d/%d us,\
...
int sensor_2l2_cis_compensate_gain_for_extremely_br(struct v4l2_subdev *subdev, u32 expo, u32 *again, u32 *dgain)
{
	int ret = 0;
	struct fimc_is_cis *cis;
	cis_shared_data *cis_data;
...
#else
	.cis_compensate_gain_for_extremely_br = sensor_2l2_cis_compensate_gain_for_extremely_br,
#endif
...
int cis_2l2_probe(struct i2c_client *client,
...
static int cis_2l2_remove(struct i2c_client *client)
{
	int ret = 0;
	return ret;
}

static const struct of_device_id exynos_fimc_is_cis_2l2_match[] = {
	{
		.compatible = "samsung,exynos5-fimc-is-cis-2l2",
	},
	{},
};
MODULE_DEVICE_TABLE(of, exynos_fimc_is_cis_2l2_match);

static const struct i2c_device_id cis_2l2_idt[] = {
	{ SENSOR_NAME, 0 },
	{},
};

static struct i2c_driver cis_2l2_driver = {
	.driver = {
		.name	= SENSOR_NAME,
		.owner	= THIS_MODULE,
		.of_match_table = exynos_fimc_is_cis_2l2_match
	},
	.probe	= cis_2l2_probe,
	.remove	= cis_2l2_remove,
	.id_table = cis_2l2_idt
};
module_i2c_driver(cis_2l2_driver);
