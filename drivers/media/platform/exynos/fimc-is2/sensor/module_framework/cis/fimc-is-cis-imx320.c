...
int sensor_imx320_cis_mode_change(struct v4l2_subdev *subdev, u32 mode)
{
	...
	if (!test_bit(FIMC_IS_SENSOR_PREPROCESSOR_AVAILABLE, &sensor_peri->peri_state)) {
		fimc_is_sensor_write8(cis->client, 0x3420, 0x02);
	}
#endif
p_err:
	I2C_MUTEX_UNLOCK(cis->i2c_lock);
...
int sensor_imx320_cis_stream_on(struct v4l2_subdev *subdev)
{
	...
	I2C_MUTEX_LOCK(cis->i2c_lock);
	sensor_imx320_cis_group_param_hold_func(subdev, 0x01);
...
int sensor_imx320_cis_set_exposure_time(struct v4l2_subdev *subdev, struct ae_param *target_exposure)
{
	...
	/* WDR mode off */
	if (cis_data->companion_data.wdr_mode != COMPANION_WDR_OFF) {
...
static int cis_imx320_remove(struct i2c_client *client)
{
	int ret = 0;
	return ret;
}

static const struct of_device_id exynos_fimc_is_cis_imx320_match[] = {
	{
		.compatible = "samsung,exynos5-fimc-is-cis-imx320",
	},
	{},
};
MODULE_DEVICE_TABLE(of, exynos_fimc_is_cis_imx320_match);

static const struct i2c_device_id cis_imx320_idt[] = {
	{ SENSOR_NAME, 0 },
	{},
};

static struct i2c_driver cis_imx320_driver = {
	.driver = {
		.name	= SENSOR_NAME,
		.owner	= THIS_MODULE,
		.of_match_table = exynos_fimc_is_cis_imx320_match
	},
	.probe	= cis_imx320_probe,
	.remove	= cis_imx320_remove,
	.id_table = cis_imx320_idt
};
module_i2c_driver(cis_imx320_driver);
