...
int fimc_is_ois_fw_version(struct fimc_is_ois *ois)
{
	int ret = 0;
	char version[7] = {0, };
...
int fimc_is_ois_fw_ready(struct fimc_is_ois *ois)
{
	...
	if (ois_status32 == OIS_FW_UPDATE_ERROR_STATUS) {
		...
		ret = fimc_is_ois_fw_version(ois);
		if (ret < 0) {
			err("Failed to ois read cal");
			return ret;
		}
...
int fimc_is_ois_mode_change(struct v4l2_subdev *subdev, int mode)
{
	int ret = 0;
	struct fimc_is_ois *ois;
	...
	case OPTICAL_STABILIZATION_MODE_STILL_ZOOM:
		fimc_is_ois_write(client, 0x0002, 0x13);
		fimc_is_ois_write(client, 0x0000, 0x01);
		break;
	...
	case OPTICAL_STABILIZATION_MODE_VDIS:
		write_data[0] = 0xFF;
		write_data[1] = 0xFF;
		fimc_is_ois_write_multi(client, 0x005E, write_data, 2);
		fimc_is_ois_write(client, 0x0002, 0x14);
		fimc_is_ois_write(client, 0x0000, 0x01);
		break;
	default:
...
static struct fimc_is_ois_ops ois_ops = {
	...
	.ois_mode_change = fimc_is_ois_mode_change,
