...
module_param(napi_weight, int, S_IRUGO | S_IWUSR);

static long gro_flush_time = 10000L;
...
#ifdef CONFIG_LINK_DEVICE_NAPI
static void gro_flush_timer(struct io_device *iod)
{
        ...
        if (unlikely(iod->flush_time.tv_sec == 0)) {
                getnstimeofday(&iod->flush_time);
        } else {
                getnstimeofday(&(curr));
                diff = timespec_sub(curr, iod->flush_time);
                if ((diff.tv_sec > 0) || (diff.tv_nsec > gro_flush_time)) {
                        napi_gro_flush(&iod->napi, false);
                        getnstimeofday(&iod->flush_time);
                }
        }
}
...
#ifdef CONFIG_LINK_DEVICE_NAPI
#if 0
        /* For napi test with GRO*/
        ret = napi_gro_receive(&iod->napi, skb);

        if (ret == GRO_DROP) {
                mif_err_limited("%s: %s<-%s: ERR! napi_gro_receive fail\n",
