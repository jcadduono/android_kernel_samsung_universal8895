...
static int decon_reset_panel(void *data)
{
	...
	struct panel_state *panel_state = decon->panel_state[0];
...
int decon_v4l2_panel_dev_ioctl(struct v4l2_subdev *sd, unsigned int cmd, void *arg)
{
...
static int get_panel_v4l2_subdev(struct decon_device *decon)
{
	...
	panel_sub = decon->panel_sd[0];

	ret = decon_v4l2_panel_dev_ioctl(panel_sub,
			PANEL_IOC_DECON_PROBE, NULL);
	if (ret) {
		decon_err("DECON:ERR:%s:failed to get panel info\n",
			__func__);
		return ret;
	}
	decon->lcd_info =
		(struct decon_lcd *)v4l2_get_subdev_hostdata(panel_sub);
	...
	for(i = 0; i < panel_cnt; i++) {
		...
		decon->panel_state[i] =
			(struct panel_state *)v4l2_get_subdev_hostdata(panel_sub);
		if (IS_ERR_OR_NULL(decon->panel_state[i])) {
			decon_err("DECON:ERR:%s:failed to get panel status\n",
				__func__);
			return -EINVAL;
		}

		v4l2_set_subdev_hostdata(panel_sub, &cb);
		ret = decon_v4l2_panel_dev_ioctl(panel_sub,
			PANEL_IOC_REG_RESET_CB, NULL);
		if (ret) {
			decon_err("DECON:ERR:%s:failed to set reset callback\n",
				__func__);
			return ret;
		}
	}
