...
int dsim_reg_enable_pll(u32 id, u32 en)
{
	...
	if (en) {
		...
		while (cnt--) {
			cnt--;
			ret = dsim_reg_is_pll_stable(id);
			if (ret)
				return 0;

			if (cnt == 0) {
				dsim_err("ERR:%s:failed to set pll\n", __func__);
				return -EBUSY;
			}
			udelay(10);
		}
	...
	if (en) {
		...
		switch(lane) {
			...
			case 4 :
				...
				break;
			default:
				dsim_err("DSIM:ERR:%s:invalid lane count : %d\n", __func__, lane);
				break;
		}
		/* set for clock lane */
		val |= (1 << 4);

		val = DSIM_CLK_CTRL_LANE_ESCCLK_EN(val);
	}
...
void dsim_reg_set_size_of_slice(u32 id, struct decon_lcd *lcd_info)
{
	...
	xres = lcd_info->xres;
	if ((lcd_info->op_mode == DSI_SDDD_MODE) || (lcd_info->op_mode == DSI_DDDD_MODE))
		xres = lcd_info->xres / 2;

	 slice_w = xres / lcd_info->dsc_slice_num;
...
void dsim_reg_set_cmd_ctrl(u32 id, struct decon_lcd *lcd_info, struct dsim_clks *clks)
{
	...
	if ((lcd_info->op_mode == DSI_SDDD_MODE) ||
		(lcd_info->op_mode == DSI_DDDD_MODE))
		xres = xres / 2;
	time_stable_vfp = xres * DSIM_STABLE_VFP_VALUE * 3 / 100;
		...
		struct dsim_clks *clks)
{
	...
	dsim_reg_set_vresol(id, lcd_info->yres);
	dsim_reg_set_hresol(id, xres, lcd_info);
	dsim_reg_set_porch(id, lcd_info);

	if (lcd_info->mode == DECON_MIPI_COMMAND_MODE){
		if (lcd_info->dsc_enabled)
			num_of_transfer = xres * lcd_info->yres / threshold / 3;
		else
			num_of_transfer = xres * lcd_info->yres / threshold;
