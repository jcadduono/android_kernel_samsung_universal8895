...
#include "../dual_panel/panel_drv.h"
...
#define PRIMARY_PANEL_SD_NAME "panel-sd.0"
#define SECONDARY_PANEL_SD_NAME "panel-sd.1"
...
static int __find_panel_v4l2_subdev(struct device *dev, void *data)
{
	...
	if (!strcmp(PRIMARY_PANEL_SD_NAME, panel->sd.name)) {
		decon->panel_sd[0] = &panel->sd;
		dsim_info("DSIM:INFO:matched panel drv name : %s\n", decon->panel_sd[0]->name);
	}
	if (decon->dt.dsi_mode == DSI_MODE_DUAL_DSI) {
		if (!strcmp(SECONDARY_PANEL_SD_NAME, panel->sd.name)) {
			decon->panel_sd[1] = &panel->sd;
			dsim_info("DSIM:INFO:matched panel drv name : %s\n", decon->panel_sd[1]->name);
		}
	}
	return 0;
}
...
int dpu_get_sd_by_drvname(struct decon_device *decon, char *drvname)
{
	...
	if (!strcmp(drvname, PANEL_DRV_NAME))
		dev = driver_find_device(drv, NULL, decon, __find_panel_v4l2_subdev);
	else
...
void decon_to_init_param(struct decon_device *decon, struct decon_param *p)
{
	...
	decon_info("%s: psr(%d) trig(%d) dsi(%d) out(%d) wins(%d) LCD[%d %d]\n",
