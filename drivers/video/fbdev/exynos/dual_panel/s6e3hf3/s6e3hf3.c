/*
 * linux/drivers/video/fbdev/exynos/panel/s6e3hf3/s6e3hf3.c
 *
 * S6E3HF3 Dimming Driver
 *
...
int getidx_brt_tbl(struct maptbl *tbl)
{
	...
	if (index > S6E3HF3_TOTAL_PAC_STEPS - 1)
		index = S6E3HF3_TOTAL_PAC_STEPS - 1;
...
int init_mdnie_night_mode_table(struct maptbl *tbl)
{
	...
	if (sizeof_maptbl(tbl) < (S6E3HF3_NIGHT_MODE_OFS +
	...
	maptbl_copy(night_maptbl, &tbl->arr[S6E3HF3_NIGHT_MODE_OFS]);
...
{
	int ret;
	u8 err[S6E3HF3_ERR_LEN] = { 0, }, err_15_8, err_7_0;
