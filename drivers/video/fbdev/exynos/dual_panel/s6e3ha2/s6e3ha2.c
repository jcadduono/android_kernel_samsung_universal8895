/*
 * linux/drivers/video/fbdev/exynos/panel/s6e3ha2/s6e3ha2.c
 *
 * S6E3HA2 Dimming Driver
 *
		...
		s32 (*mtp)[MAX_COLOR], s32 (*hbm)[MAX_COLOR])
{
	...
	int nr_tp;
	u8 gamma_out[GAMMA_CMD_CNT - 1] = { 0, };
	...
	for (i = 0; i < brt_tbl->sz_lum; i++) {
		...
		get_dimming_gamma_compact(dim_info, luminance, gamma_out);
		memcpy((u8 *)&gamma_maptbl->arr[i * gamma_maptbl->ncol], gamma_out, nr_tp * 3);
		gamma_maptbl->arr[i * gamma_maptbl->ncol + (nr_tp * 3)] = 0;
		gamma_maptbl->arr[i * gamma_maptbl->ncol + (nr_tp * 3 + 1)] = 0;
		gamma_maptbl->arr[i * gamma_maptbl->ncol + (nr_tp * 3 + 2)] = 0;
		memcpy((u8 *)&gamma_maptbl->arr[i * gamma_maptbl->ncol + ((nr_tp + 1) * 3)],
				gamma_out + (nr_tp * 3), 2);
	}
...
static int init_subdev_gamma_table(struct maptbl *tbl, int id)
{
	...
	ret = init_dimming(panel_data, id, mtp_offset, NULL);
	if (unlikely(ret)) {
...
int getidx_brt_tbl(struct maptbl *tbl)
{
	...
	if (index > S6E3HA2_TOTAL_PAC_STEPS - 1)
		index = S6E3HA2_TOTAL_PAC_STEPS - 1;
...
int getidx_dd_sel_table(struct maptbl *tbl)
{
	return 0;
}
...
int init_mdnie_night_mode_table(struct maptbl *tbl)
{
	...
	if (sizeof_maptbl(tbl) < (S6E3HA2_NIGHT_MODE_OFS +
	...
	maptbl_copy(night_maptbl, &tbl->arr[S6E3HA2_NIGHT_MODE_OFS]);
...
{
	int ret;
	u8 err[S6E3HA2_ERR_LEN] = { 0, }, err_15_8, err_7_0;
