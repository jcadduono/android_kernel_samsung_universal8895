...
#ifdef CONFIG_HPA
	if (*flags & BASE_MEM_SECURE) {
		*va_pages = PAGE_ALIGN((ION_HPA_PAGE_COUNT(dma_buf->size) * ION_HPA_DEFAULT_SIZE)) >> PAGE_SHIFT;
	}
	else
#endif
		*va_pages = PAGE_ALIGN(dma_buf->size) >> PAGE_SHIFT;
#else /* MALI_SEC_SECURE_RENDERING : below code is orignal */
	*va_pages = PAGE_ALIGN(dma_buf->size) >> PAGE_SHIFT;
#endif
	if (!*va_pages)
		goto bad_size;
...
int kbase_mmap(struct file *file, struct vm_area_struct *vma)
{
	...
	case PFN_DOWN(BASE_MEM_COOKIE_BASE) ...
		...
		/*
		 * Overwrite the offset with the region start_pfn, so we
		 * effectively map from offset 0 in the region. However
		 * subtract the aligned offset so that when user space trims
		 * the mapping the beginning of the trimmed VMA has the correct
		 * vm_pgoff.
		 */
