/* linux/drivers/devfreq/exynos/exynos8895_bus_cam.c
 *
 * Copyright (c) 2015 Samsung Electronics Co., Ltd.
 *              http://www.samsung.com
 *
 * Samsung EXYNOS8895 SoC CAM devfreq driver
 *
...
static int exynos8895_devfreq_cam_reboot(struct exynos_devfreq_data *data)
{
...
static int exynos8895_devfreq_cam_get_freq(struct device *dev, u32 *cur_freq,
...
static int exynos8895_devfreq_cam_set_freq(struct device *dev, u32 new_freq,
		struct clk *clk, struct exynos_devfreq_data *data)
{
...
static int exynos8895_devfreq_cam_init_freq_table(struct exynos_devfreq_data *data)
{
...
static int __init exynos8895_devfreq_cam_init_prepare(struct exynos_devfreq_data *data)
{
	...
	data->ops.init_freq_table = exynos8895_devfreq_cam_init_freq_table;
	data->ops.reboot = exynos8895_devfreq_cam_reboot;
	data->ops.cmu_dump = exynos8895_devfreq_cam_cmu_dump;

	return 0;
}

static int __init exynos8895_devfreq_cam_initcall(void)
{
	if (register_exynos_devfreq_init_prepare(DEVFREQ_CAM,
				exynos8895_devfreq_cam_init_prepare))
		return -EINVAL;

	return 0;
}
fs_initcall(exynos8895_devfreq_cam_initcall);
